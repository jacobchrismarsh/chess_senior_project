...
class ChatManager(GObject.GObject):
    ...
    def __init__(self, connection):
        ...
        self.connection.expect_line(self.onAnnouncement, "\s*\*\*ANNOUNCEMENT\*\* (.*)")
        self.connection.expect_line(
            self.onChannelMessage, "%s(\*)?\((\d+)\): (.*)" % names
        )
        self.connection.expect_line(
            self.onShoutMessage, "%s(\*)? (c-)?shouts: (.*)" % names
        )
        self.connection.expect_line(self.onShoutMessage, "--> %s(\*)?:? (.*)" % names)
        self.connection.expect_line(
            self.onKibitzMessage, "%s%s\[(\d+)\] kibitzes: (.*)" % (names, ratings)
        )
        self.connection.expect_line(
            self.onWhisperMessage, "%s%s\[(\d+)\] whispers: (.*)" % (names, ratings)
        )

        self.connection.expect_line(
            self.onArrivalNotification, "Notification: %s has arrived\." % names
        )
        self.connection.expect_line(
            self.onDepartedNotification, "Notification: %s has departed\." % names
        )
        ...
        self.connection.expect_line(
            self.getNoChannelPlayers, "Channel (\d+) is empty\."
        )
        self.connection.expect_fromto(
        ...
        self.connection.expect_line(
            self.get_allob_list, "(?:Observing|Examining)\s+(\d+).+: (.+) \("
        )
