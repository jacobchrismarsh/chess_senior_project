...
class ICCAdjournManager(AdjournManager):
    ...
    def on_icc_gamelist_item(self, data):
        ...
        idx, gid, event, date, time, wname, wrating, bname, brating, rated, rating_type, wild, wtime, winc, btime, binc, eco, status, color, mode, rest = data.split(
            " ", 20
        )
    ...
    def queryMoves(self, game):
        ...
        if isinstance(game, FICSHistoryGame):
            self.connection.client.run_command(
                "spgn %s %s" % (self.connection.history_owner, game.history_no)
            )
        elif isinstance(game, FICSJournalGame):
            self.connection.client.run_command(
                "spgn %s %%%s" % (self.connection.journal_owner, game.journal_no)
            )
        else:
            self.connection.client.run_command(
                "spgn %s %s" % (self.connection.stored_owner, game.opponent.name)
            )
