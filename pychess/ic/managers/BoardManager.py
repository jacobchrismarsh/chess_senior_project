...
class BoardManager(GObject.GObject):
    ...
    __gsignals__ = {
        ...
        "timesUpdate": (GObject.SignalFlags.RUN_FIRST, None, (int, int, int)),
    ...
    def __init__(self, connection):
        ...
        self.connection.expect_line(
            self.onWasPrivate, "Sorry, game (\d+) is a private game\."
        )
        self.connection.expect_line(
        ...
        self.connection.expect_line(
            self.player_lagged, "Game (\d+): %s has lagged for (\d+) seconds\." % names
        )
        self.connection.expect_line(
        ...
        self.connection.expect_line(
            self.onUnobserveGame, "Removing game (\d+) from observation list\."
        )

        self.connection.expect_line(
            self.made_examined, "%s has made you an examiner of game (\d+)\." % names
        )

        self.connection.expect_line(
            self.made_unexamined, "You are no longer examining game (\d+)\."
        )
    ...
    def onStyle12(self, match):
        ...
        if gameno in self.castleSigns:
            ...
            castleSigns = ("k", "q")
        gameno, relation, curcol, ply, wname, bname, wms, bms, gain, lastmove, fen = self.parseStyle12(
            style12, castleSigns
        )
        ...
        if lastmove is None and relation == IC_POS_EXAMINATING:
            ...
            if self.connection.examined_game is None:
                ...
                log.debug(
                    "emit('boardSetup') with %s %s %s %s" % (gameno, fen, wname, bname),
                    extra={"task": (self.connection.username, "BM.onStyle12")},
                )
    ...
    def parseGame(self, matchlist, gameclass, in_progress=False, gameno=None):
        ...
        for line in matchlist[movesstart:-1]:
            ...
            moveno, wmove, whour, wmin, wsec, wmsec, bmove, bhour, bmin, bsec, bmsec = moveListMoves.match(
                line
            ).groups()
